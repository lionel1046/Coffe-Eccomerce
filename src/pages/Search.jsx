...
const Search = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await axios(
        `https://json-server-production-d0c3.up.railway.app/products?q=${e.target.search.value}&_page=${currentPage}`
      );
  ...
  const handleSearchPagination = async () => {
    try {
      const response = await axios(
        `https://json-server-production-d0c3.up.railway.app/products?q=${searchTerm}&_page=${currentPage}`
      );
      const data = response.data;
      setProducts(data);
    } catch (error) {
